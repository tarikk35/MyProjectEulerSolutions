# -*- coding: utf-8 -*-
"""
Created on Tue Dec 11 21:52:35 2018

@author: Dell

Find the thirteen adjacent digits in the 1000-digit number that have the greatest product.
 What is the value of this product?
"""
# first loop splits text into pieces and makes it one digit number.
# then i check that if there is 13 items splitted
# if yes, second loop iterates through last splitted digit's index - 13 to last splitted digit's index
# so my processor would be grateful to me.
# and then i check if 13 adjacent number's multiplication is bigger than previous ones.
# if yes, change it.
# and reset temporary values.
# this solution works for any properly given text and length.

def FindGreatest(text,length):
    numberList,maxValue,adjMultiplied,luckyNumbers=[],0,1,[]
    for i in range(0,len(text),1):
        numberList.append(int(text[i:i+1]))
        if i>13:
            for j in range(i-13,i,1):
                adjMultiplied*=numberList[j]
                luckyNumbers.append(numberList[j])
            if adjMultiplied>maxValue:
                print(str(luckyNumbers))
                print(adjMultiplied)
                maxValue=adjMultiplied
            adjMultiplied=1
            luckyNumbers=[]
# i made it string because this number is bigger than a googol and i can split it easily
text="" 
FindGreatest(text,13)